...
export default function Gallery() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
      {images.map((src, i) => (
        <div key={i} className="card" style={{ overflow: 'hidden', cursor: 'pointer', position: 'relative' }} onClick={() => setOpenIdx(openIdx === i ? null : i)}>
          <img
            src={src}
            alt={`Шторы ${i+1}`}
            style={{ width: '100%', height: openIdx === i ? 340 : 180, objectFit: 'cover', display: 'block', borderRadius: 8, transition: 'height 0.3s' }}
          />
          {openIdx === i && (
